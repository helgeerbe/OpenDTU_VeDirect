...
void Provider::pollingLoop()
{
    ...
    while (!_stopPolling) {
        ...
        dataInFlight.add<DataPointLabel::Export>(energyExport);

        if (_phases == Phases::Three) {
            dataInFlight.add<DataPointLabel::PowerL2>(phase2Power);
            dataInFlight.add<DataPointLabel::PowerL3>(phase3Power);
            dataInFlight.add<DataPointLabel::VoltageL2>(phase2Voltage);
            dataInFlight.add<DataPointLabel::VoltageL3>(phase3Voltage);
        }
